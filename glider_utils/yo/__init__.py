...
def interpolate_depth(est_data, interval):
    ts = np.arange(
        ...
        np.max(est_data[:, TIME_DIM]) + interval,
        interval
    )
...
def calculate_delta_depth(interp_data, interval):
    ...
    delta_depth = boxcar_smooth_data(delta_depth, math.ceil(interval/2))

    return delta_depth


def create_profile_entry(dataset, start, end):
    time_start = dataset[start, 0]
    time_end = dataset[end, 0]
    depth_start = dataset[start, 1]
    depth_end = dataset[end, 1]
    return {
