...
ENV GUTILS_BINARY_DIRECTORY /binary
VOLUME $GUTILS_BINARY_DIRECTORY

ENV GUTILS_ASCII_DIRECTORY /ascii
VOLUME $GUTILS_ASCII_DIRECTORY

ENV GUTILS_NETCDF_DIRECTORY /netcdf
VOLUME $GUTILS_NETCDF_DIRECTORY

ENV GUTILS_CONFIG_DIRECTORY /config
VOLUME $GUTILS_CONFIG_DIRECTORY

RUN mkdir -p /etc/my_init.d
COPY docker/init/* /etc/my_init.d/

ENV GUTILS_VERSION 2.0.0
